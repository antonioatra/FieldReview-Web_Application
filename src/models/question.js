...
module.exports = {
  ...
  async findById(id) {
    // cria a função para encontrar perguntas pelo id
  ...
  async findByModuloId(data) {
    // retorna perguntas de acordo com o módulo que a pergunta está contida
  ...
  async update(id, data) {
    // atualiza pergunta
    const query = 'UPDATE pergunta SET enunciado = $1, pontos = $2 WHERE id = $3 RETURNING *';
    const values = [data.statement, data.points, id];

    const result = await pool.query(query, values);
    return result.rows[0]; // retorna a pergunta atualizada
  },
