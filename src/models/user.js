...
module.exports = {
  // Criando as funções do model

  async create(data) {
    // Função que cria os usuários
    const query = 'INSERT INTO usuarios(nome, email, senha, cargo) VALUES($1,$2,$3,$4)'; // Cria os usuários dentro do banco de dados
    const values = [data.name, data.email, data.password, data.role]; // Parâmetros que serão adicionados a esse usuário no banco de dados
  ...
  async findById(id) {
    // Função que pega os usuários por id
